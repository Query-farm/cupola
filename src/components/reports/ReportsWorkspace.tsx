...
const materializedMapRows = new WeakMap<ArrowTable, Map<string, Record<string, any>[]>>();

/** Map rows for a decoded result, memoized per table and geometry column.
 *  `ReportMap` rebuilds its Leaflet map whenever `rows` changes identity, so
 *  building them inline in render tore the map down on every workspace render
 *  — including the one a marker click causes by selecting its block, which
 *  closed the popup that click had just opened — and re-formatted every row of
 *  the dataset each time. */
function datasetMapRows(table: ArrowTable, geometryColumn?: string): Record<string, any>[] {
  let byGeometry = materializedMapRows.get(table);
  if (!byGeometry) materializedMapRows.set(table, byGeometry = new Map());
  const key = geometryColumn ?? "";
  let rows = byGeometry.get(key);
  if (!rows) byGeometry.set(key, rows = reportMapRows(table, geometryColumn));
  return rows;
}
...
export function ReportsWorkspace({ catalogData, serviceUrl, attachedCatalogNames = [], attachedCatalogs = [], onBusyChange, initialReport }: Props) {
  ...
  // measureBeforeMount keeps `mounted` false until the canvas has really been
  // measured. Without it the grid first laid out at `initialWidth` and then
  // animated every block to the true width on each report open, while the
  // (unanimated) group frames jumped straight there — blocks overhung their
...
Choose parameter types for the appropriate reader controls: date gives a native calendar picker; date_range gives start/end calendar pickers and date presets; select gives a dropdown; multi_select gives multiple-choice checkboxes; boolean gives a checkbox; number gives a numeric input; text gives a text box. Use static options or dataset-backed options for select and multi_select. Prefer these typed controls over text boxes for dates, choices, numbers, and booleans. Parameters are a validated public interface, not merely SQL substitutions. Set required when empty input is invalid. Use validation for type-appropriate declarative constraints: number min/max/exclusiveMin/exclusiveMax/step/integer; text minLength/maxLength/pattern; date min/max; date_range min/max/requireBoth/maxSpanDays; multi_select minSelections/maxSelections. Static and dataset-backed select values are checked for membership when Apply is pressed. Use parameterRules for relationships between values, including date_range paths such as period.start and period.end. A rule has leftKey, operator, exactly one of rightKey or value, and a reader-friendly message. For data-dependent business rules, create a role="parameter_validation" dataset that returns one row with a boolean column and optional message column, then reference it from validationDataset on the parameter. Keep validation SQL read-only and parameterized. Reader text supports $key, $key_label, $key_value, and date-range $key_start/$key_end; use $$ for a literal dollar sign. Do not invent arbitrary JavaScript validation or interpolate raw SQL fragments.
