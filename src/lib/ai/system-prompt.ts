...
export function buildSystemPrompt(
  ...
  const lines: string[] = [
    ...
    `* **query_semantic_model** — Compile and execute modeled measures and dimensions. Prefer it when the requested concepts exist in semantic tags. It supports bounded correlated inputs, multi-fact stitching, explicit conformed branch members, model-owned measure filters, and typed cross-fact derived measures. Every base input to a cross-fact formula needs an explicit missing_fact_value. Use compile_only to inspect generated SQL without running it.`,
