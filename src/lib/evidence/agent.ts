...
export const editableFields = ['title', 'source', 'setupSql', 'parameters', 'values', 'drillPaths', 'appearance', 'semanticDatasets', 'pivots'] as const;
...
const parameterValueSchema = { anyOf: [scalar, { type: 'array', items: { type: ['string', 'number'] } }, { type: 'object', additionalProperties: false, required: ['start', 'end'], properties: { start: { type: ['string', 'null'] }, end: { type: ['string', 'null'] } } }] };
const parameterToolSchema = { type: 'object', additionalProperties: false, required: ['id', 'key', 'label', 'type', 'required', 'defaultValue'], properties: {
  id: { type: 'string' }, key: { type: 'string' }, label: { type: 'string' }, description: { type: 'string' },
  type: { enum: ['text', 'number', 'date', 'boolean', 'select', 'multi_select', 'date_range'] }, required: { type: 'boolean' }, defaultValue: parameterValueSchema,
  options: { anyOf: [
    { type: 'object', additionalProperties: false, required: ['kind', 'values'], properties: { kind: { enum: ['static'] }, values: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['value', 'label'], properties: { value: { type: ['string', 'number'] }, label: { type: 'string' } } } } } },
    { type: 'object', additionalProperties: false, required: ['kind', 'sql'], properties: { kind: { enum: ['query'] }, sql: { type: 'string' }, valueColumn: { type: 'string' }, labelColumn: { type: 'string' } } },
  ] },
...
export const EVIDENCE_AGENT_TOOLS: Tool[] = [
  ...
  { name: 'preview_parameter_options', description: 'Run a select / multi_select parameter\'s choices query against the current report values and return the choice count and a sample. Pass `key` for an existing parameter, or `parameter` for a draft definition (it may use other parameters of the current report). Read-only.', input_schema: {
    type: 'object', additionalProperties: false, properties: { key: { type: 'string' }, parameter: parameterToolSchema },
  } },
