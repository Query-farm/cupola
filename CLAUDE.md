...
**The capability probe is rewritten from OPTIONS to HEAD** (`src/lib/vgi-fetch.ts`): the TypeScript client discovers a server's HTTP capabilities by fetching `OPTIONS {prefix}/health` and reading the capability headers off the reply. From a browser that can never work — the custom `VGI-Accept-Max-Response-Bytes` header makes it non-simple, so the browser preflights it with `Access-Control-Request-Method: OPTIONS`, and the Python server's `/health` is a Falcon resource implementing `on_get`/`on_head`, so CORSMiddleware answers `Access-Control-Allow-Methods: GET, HEAD` and the probe is blocked before it is sent. The failure names a method nobody wrote and kills the connection at its first hop. `HEAD` is the same probe without the problem (the server implements it for exactly this, and the capability headers ride every response and are named in `Access-Control-Expose-Headers`), so cupola passes `httpConnect` a `fetch` that rewrites that one request shape. **This belongs upstream in the client** — every browser consumer of it hits this.
