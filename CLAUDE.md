...
- **History must stay append-only.** Rewriting an earlier turn is the same invalidation in a different place. Earlier report snapshots keep their own values rather than being updated, which is why they are labelled "at the start of this turn". `sanitizeConversation` is exempt only because it no-ops on a healthy conversation. **`pruneCarriedToolImages` still violates this** — it rewrites an earlier `tool_result` on every round, and carrying a ~1.5k-token image at the cache-read rate is far cheaper than re-writing the tail behind it. It is a context-limit measure that currently fires unconditionally.
