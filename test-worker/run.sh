...
#   ./run.sh --latest        # newest vgi-python, ignoring the script's pins
set -e
cd "$(dirname "$0")"
...
if [ "$1" = "--latest" ]; then
  shift
  # stress_worker.py's inline metadata pins the current releases. Running it
  # through `python` rather than as the uv script target skips that metadata,
  # so this resolves the newest worker published right now. Cupola installs the
  # VGI extension unpinned, so the default already tracks it — this is for
  # checking a release that landed after the pins were last moved.
