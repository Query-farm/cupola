...
describe('the page frame', () => {
  test('the template is ASCII, so no transpiler can turn a character into a literal escape', () => {
    expect(REPORT_TEMPLATE).toMatch(/^[\x00-\x7F]*$/);
  });
  test('pages carry only the report: no title block or header; the footer has the date, the credit and page numbers', async () => {
    const { getDocument } = await import('pdfjs-dist/legacy/build/pdf.mjs');
    const t = (value: string) => [{ kind: 'text' as const, text: value }];
    ...
    const pages = await Promise.all(Array.from({ length: pdf.numPages }, async (_, i) => {
      ...
      const links = (await page.getAnnotations()).map(annotation => annotation.url).filter(Boolean);
      return { text, links };
    }));
    expect(pages).toHaveLength(2);
    // The report's own heading opens the page; the title only names the document.
    expect(pages[0].text.trim().startsWith('Sales overview')).toBe(true);
    expect(pages.map(page => page.text).join(' ')).not.toContain('Quarterly sales');
    expect((await pdf.getMetadata()).info).toMatchObject({ Title: 'Quarterly sales' });
    for (const [i, page] of pages.entries()) {
      expect(page.text).toContain('September 26, 2026 at 4:52 PM · Generated by Cupola by Query.Farm');
